...
echo $content 
echo "--------------------------------------------"
echo "Check OK if content contains string : [Put to insert data in MongoDB!!!]"
if [[ $content =~ .*'Put to insert data in MongoDB!!!'.* ]]
then
	echo "[MESSAGE TEST] GET IS OK"
	echo ">>>>> TEST PUT METHOD OF DESTEST SERVLET <<<<<"
	contentPUT= $(curl -X PUT -H "Accept: application/json" -H "Content-type: application/json" -d '{"sourceJob":"abc", "istIid": "def"}' http://localhost:13111/spool/tfm_test -v)
